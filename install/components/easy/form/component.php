...
if (Loader::IncludeModule("easy.form")) {
    $captchaKey = Option::get("easy.form", "CAPTCHA_KEY", "", SITE_ID);
    $captchaSecretKey = Option::get("easy.form", "CAPTCHA_SECRET_KEY", "", SITE_ID);
}
...
$arParams['USER_EMAIL'] = '';
$arParams['EMAIL_TO'] = trim($arParams['EMAIL_TO']) ? trim($arParams['EMAIL_TO']) : Option::get("easy.form", "EMAIL", "", SITE_ID);
$arParams['BCC'] = trim($arParams['BCC']);

$arParams['OK_TEXT'] = $arParams['OK_TEXT'] ? htmlspecialcharsback(trim($arParams['OK_TEXT'])) : Loc::getMessage('EASY_FORM_MESS_OK_TEXT');
$arParams['ERROR_TEXT'] = $arParams['ERROR_TEXT'] ? htmlspecialcharsback(trim($arParams['ERROR_TEXT'])) : Loc::getMessage('EASY_FORM_MESS_ERROR_TEXT');
...
if ($arParams['USE_MODULE_VARNING'] != 'N' && Loader::includeModule("easy.form")) {
    if (Option::get("easy.form", "SHOW_MESSAGE", "", SITE_ID) == 'Y') {
        $mess = Option::get("easy.form", "MESSAGE_TEXT", "", SITE_ID);
        if (strlen($mess) > 0) {
...
if ($request->isPost() && $arParams['FORM_ID'] == $request['FORM_ID']) {
    ...
    if ($arParams["USE_CAPTCHA"]) {
        ...
        if (isset($_REQUEST['g-recaptcha-response'])) {
            ...
            if ($resp->isSuccess()) {
                ...
                $arMessageError[] = Loc::getMessage('EASY_FORM_CAPTCHA_ERROR');
            }
        }
    }
        ...
        if (is_array($field)) {
            ...
            if ($fieldInfo['TYPE'] == 'file') {
                ...
                if (!empty($field['tmp_name'])) {
                    ...
                    $f_id = \CFile::SaveFile($field, 'easy.form');
                    if ($f_id)
                        $fieldFiles[] = $f_id;
                } else {
                    $fieldFiles = $field;
    ...
    if ((!empty($arPostFields['EMAIL']) || in_array('EMAIL', $arParams['REQUIRED_FIELDS'])) && !check_email($arPostFields['EMAIL']))
        $arMessageError[] = Loc::getMessage('EASY_FORM_EMAIL_ERROR');
    ...
    if ($emptyForm) {
        $arMessageError[] = Loc::getMessage('EASY_FORM_EMPTY_FORM_ERROR');
    }
    ...
    if (!in_array('NONE', $arParams['REQUIRED_FIELDS'])) {
        foreach ($arParams['REQUIRED_FIELDS'] as $FIELD) {
            ...
            if ((empty($arParams['REQUIRED_FIELDS']) || in_array($FIELD, $arParams['REQUIRED_FIELDS'])) && strlen($arPostFields[$FIELD]) == 0) {
                $arMessageError[] = Loc::getMessage("EASY_FORM_FIELD_ERROR_MESS", array("#FIELD_NAME#" => $message_field));
            }
        }
        unset($FIELD);
    }
    ...
    if (empty($arMessageError) && $arParams['ENABLE_SEND_MAIL']) {
        ...
        if ($arPostFields) {
            ...
            foreach ($arPostFields as $code => $val) {
                ...
                foreach (GetModuleEvents("easy.form", "OnBeforeFieldsValue", true) as $arEvent)
    ...
    if (empty($arMessageError) && $arParams['USE_IBLOCK_WRITE'] == 'Y' && intval($arParams['IBLOCK_ID']) > 0 && Loader::IncludeModule("iblock")) {
        ...
        foreach (GetModuleEvents("easy.form", "OnBeforeIBlockElementAdd", true) as $arEvent)
        ...
        if ($idElement = $el->Add($arLoadProductArray)) {
            ...
            foreach (GetModuleEvents("easy.form", "OnAfterIBlockElementAdd", true) as $arEvent)
	...
	if (Loader::IncludeModule("easy.form") && empty($arMessageError)) {
        $fields = array();
		...
		if (is_array($arParams['EVENT_MESSAGE_ID']) && !empty($arParams['EVENT_MESSAGE_ID'])) {
            foreach ($arParams['EVENT_MESSAGE_ID'] as $arMessID) {
                if (!CEvent::Send($arParams['EVENT_NAME'], SITE_ID, $arServiceFields, 'N', $arMessID, $file_id)) {
                    $arMessageError[] = Loc::getMessage('EASY_FORM_SEND_ERROR_MESS', array("#MESS_ID#" => $arMessID));
                }
            }
        } else {
            if (!CEvent::Send($arParams['EVENT_NAME'], SITE_ID, $arServiceFields, 'N', "",  $file_id)) {
                $arMessageError[] = Loc::getMessage('EASY_FORM_SEND_ERROR');
            }
        }
    ...
    foreach (GetModuleEvents("easy.form", "OnAfterAdditionalMessage", true) as $arEvent)
...
if ($arParams["USE_CAPTCHA"] && !defined("EASY_FORM_RE_CAPTCHA_INCLUDE")) {
    $APPLICATION->AddHeadScript('https://www.google.com/recaptcha/api.js');
    define("EASY_FORM_RE_CAPTCHA_INCLUDE", "Y");
}
...
if ($arParams["USE_BOOTSRAP_CSS"] == 'Y' && !defined("EASY_FORM_BOOTSTRAP_CSS_INCLUDE")) {
    $APPLICATION->SetAdditionalCSS($componentPath . '/lib/css/bootstrap.min.css');
    define("EASY_FORM_BOOTSTRAP_CSS_INCLUDE", "Y");
}

if ($arParams['USE_FORMVALIDATION_JS'] == "Y" && $arParams['INCLUDE_BOOTSRAP_JS'] != "N" && !defined("EASY_FORM_BOOTSTRAP_VALIDATOR_INCLUDE")) {
    ...
    define("EASY_FORM_BOOTSTRAP_VALIDATOR_INCLUDE", "Y");
}

if ($arParams["USE_INPUTMASK_JS"] == 'Y' && !defined("EASY_FORM_FORM_INPUTMASK_INCLUDE")) {
    $APPLICATION->AddHeadScript($componentPath . '/lib/js/inputmask.js');
    define("EASY_FORM_FORM_INPUTMASK_INCLUDE", "Y");
}

if (!defined("EASY_FORM_FORM_JS_INCLUDE")) {
    $APPLICATION->AddHeadScript($componentPath . '/script.js');
    define("EASY_FORM_FORM_JS_INCLUDE", "Y");
}
