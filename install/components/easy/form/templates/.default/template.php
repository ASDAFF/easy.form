...
<div class="easy-form<?= $arParams['HIDE_FORMVALIDATION_TEXT'] == 'Y' ? ' hide-formvalidation' : '' ?>" <?= $WITH_FORM ?>>
    <form id="<?= $FORM_ID ?>"
            ...
            <? if (!empty($arResult['FORM_FIELDS'])) {
                            ...
                            <div class="form-group">
                                <? if (!$arParams['HIDE_FIELD_NAME']) { ?>
                                    <label class="control-label"
                                           for="<?= $arField['ID'] ?>"><?= $arField['TITLE'] ?></label>
                                <? } ?>
                                <div>
                                    <textarea class="form-control" id="<?= $arField['ID'] ?>" rows="5"
                                              name="<?= $arField['NAME'] ?>" <?= $arField['PLACEHOLDER_STR']; ?> <?= $arField['REQ_STR'] ?>><?= $arField['VALUE']; ?></textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-xs-12">
                            <div class="form-group">
                                ...
                                foreach ($arField['VALUE'] as $key => $arVal) {
                                    if (!$arField['SHOW_INLINE']) { ?>
                                        <div class="<?= $arField['TYPE'] ?>">
                                    <? }
                                    if (!empty($arVal)) { ?>
                            ...
                            <div class="form-group">
                                <label class="checkbox-inline">
                                    <input type="checkbox" name="<?= $arField['NAME'] ?>"
                                           value="<?= Loc::getMessage('EASY_FORM_YES') ?>" <?= $arField['REQ_STR'] ?>>&nbsp;<?= htmlspecialcharsBack($arField['VALUE']) ?>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="col-xs-12 switch-select">
                            <div class="form-group switch-parent">
                                ...
                                <select class="form-control"
                                        id="<?= $arField['ID'] ?>" <?= $arField['MULTISELECT'] == 'Y' ? 'multiple' : '' ?>
                                    ...
                                    if (is_array($arField['VALUE'])) {
                                        ...
                                        if ($arField['SET_ADDITION_SELECT_VAL']) { ?>
                                            <option value=""
                                                    data-switch="other"><?= $arField['ADDITION_SELECT_VAL'] ?></option>
                                        <? }
                                    } ?>
                                </select>
                            </div>
                            <? if ($arField['SET_ADDITION_SELECT_VAL']) { ?>
                                <div class="form-group switch-child hidden">
                                    <? if (!$arParams['HIDE_FIELD_NAME']) { ?>
                                        <label class="control-label"
                                               for="<?= $arField['SET_ADDITION_SELECT_ID'] ?>"><?= $arField['TITLE'] ?></label>
                                    <? } ?>
                                    <div class="row">
                                        <div class="col-xs-9">
                                            <input class="form-control" type="text"
                                                   id="<?= $arField['SET_ADDITION_SELECT_ID'] ?>"
                                                   name="<?= $arField['ADDITION_SELECT_NAME'] ?>" value=""
                                                   maxlength="30" <?= $arField['REQ_STR'] ?>>
                                        </div>
                                        <div class="col-xs-3">
                                            <a href="" class="btn-switch-back"
                                               onclick="return false;"><?= Loc::getMessage('EASY_FORM_TO_LIST') ?></a>
                                        </div>
                                    </div>
                                </div>
                            <? } ?>
                            ...
                            <div class="form-group">
                                <? if (!$arParams['HIDE_FIELD_NAME']) { ?>
                                    <label class="control-label"
                                           for="<?= $arField['ID'] ?>"><?= $arField['TITLE'] ?></label>
                                <? } ?>
                                <input class="form-control" type="<?= $arField['TYPE']; ?>" id="<?= $arField['ID'] ?>"
                                       name="<?= $arField['NAME'] ?>"
                                       value="<?= $arField['VALUE']; ?>" <?= $arField['PLACEHOLDER_STR']; ?> <?= $arField['REQ_STR'] ?> <?= $arField['MASK_STR'] ?>>
                            </div>
                        </div>
                    <? }
                } ?>
                    ...
                    <div class="col-xs-12">
                        <div class="form-group">
                            ...
                            <input id="<?= $FORM_ID ?>-captchaValidator" class="form-control" type="text" required
                                   data-bv-notempty-message="<?= GetMessage("EASY_FORM_REQUIRED_MESS"); ?>"
                ...
                <div class="col-xs-12">
                    <? if ($arResult['WARNING_MSG']) { ?>
                        <p class="warning-buy">
                            <small><?= $arResult['WARNING_MSG']; ?></small>
                        </p>
                    <? } ?>
